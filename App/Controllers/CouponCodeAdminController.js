...
module.exports = {
	...
	list: async function (req, res) {
		try {
			var data = {
				...
				couponcodeActive: 'active',
			};
			// console.log("Datat", data);
			return res.render('backend/couponcode/listCouponcode', data);
		} catch (e) {
	...
	getCouponcode: async function (req, res) {
		...
		try {
            ...
            let couponcode_data = await Sys.App.Services.CouponcodeServices.getByData({ is_deleted: "0" });    
            if(couponcode_data){
			...
			if (search != '') {
				let capital = search;
				query = { vendor_first_name: { $regex: '.*' + search + '.*' }, is_deleted: "0", is_expire: "false" };
			...
			let vendorCount = await Sys.App.Services.CouponcodeServices.getVendorCount(query);
			let data = await Sys.App.Services.CouponcodeServices.getByData(query, length, start);
			...
			console.log("Error in couponcodeAdminController in getVendor", e);
		}

	},

    addCouponcode: async function (req, res) {
		try {
            var d = new Date();
    ...
    addPostCouponcode: async function (req, res) {
        try {
            console.log("req.body :: ",req.body);
            let couponcode_get_data = await Sys.App.Services.CouponcodeServices.getVendorData({ couponcode: req.body.couponcode, is_deleted: "0" });
            console.log("couponcode_get_data :: ",couponcode_get_data);
            
            if(couponcode_get_data){
                console.log("Coupon Code Name already Exists")
                req.flash('error', "Coupon Code Name is Already Exists!");
                return res.redirect('/backend/couponcode');
            }
            let couponcode_data = await Sys.App.Services.CouponcodeServices.addVendor({
            ...
            console.log("couponcode_data create", couponcode_data);
            // console.log("PRODUCT AFTER INSERT DAta", product);
            req.flash('success','Create Successfully');
            return res.redirect('/backend/couponcode');
        } catch (error) {
            console.log("Error in CouponcodeController in addPostCouponcode", error);
        }
    },

    editCouponcode: async function (req, res) {
		try {
            console.log("req.params :: ",req.params);
            let couponcodeData = await Sys.App.Services.CouponcodeServices.getVendorData({ _id: req.params.id });
			console.log("couponcode data edit :: ",couponcodeData);
            var d = new Date();
    ...
    editPostCouponcode: async function (req, res) {
        ...
        try {
            ...
            let couponcode_get_data = await Sys.App.Services.CouponcodeServices.getVendorData({ _id: req.body.id });
            if(couponcode_get_data){
            
                let couponcode_data = await Sys.App.Services.CouponcodeServices.updateVendorData(
                    { _id: couponcode_get_data._id },{
                    couponcode: req.body.couponcode,
                    discount_amount: req.body.discount_amount,
                    couponcode_text: req.body.couponcode_text,
                    couponcode_type: req.body.couponcode_type,
    ...
    deleteCouponcode: async function (req, res) {
		try {
            console.log("req.body :: ",req.params);
            let couponcode_data = await Sys.App.Services.CouponcodeServices.getVendorData({ _id: req.params.id });
			console.log("couponcode_data delete :: ",couponcode_data);

            if(couponcode_data){
            
                await Sys.App.Services.CouponcodeServices.updateVendorData(
                    { _id: couponcode_data._id },{
                    is_deleted: "1"
                });
                res.send('success');
            }else{
