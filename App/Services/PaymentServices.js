...
const productModel = mongoose.model('payment');
module.exports = {
  ...
  getUserDataSort: async function (query,start) {
    try {
      return await userModel.find(query).skip(start).sort({updatedAt:-1});
    } catch (e) {
      console.log("Error", e);
    }
  },

  lastRecord : async function(data) {
    try {
      return await userModel.findOne(data).sort({_id:-1}).skip(1);
    } catch (e) {
      console.log("Error", e);
    }
  },
...
getProductDatatable_sort: async function(query, length, start){
    try {
        return await productModel.find(query).skip(start).sort({_id:-1}).limit(length);
    } catch (e) {
        console.log("Error in ProductServices in getProductDatatable",e);
    }
},
